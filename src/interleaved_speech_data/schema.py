...
class ManifestRecord:
    """One source utterance. Transcript and word timestamps are optional."""
...
def validate_shard(path: str | Path, verify_checksums: bool = True) -> dict[str, Any]:
    ...
    expected_audio = int(lengths.sum()) * n_codebooks * np.dtype(np.int16).itemsize
    ...
    codes = np.memmap(
        ...
        dtype=np.int16,
        mode="r",
        shape=(int(lengths.sum()), n_codebooks),
    )
    ...
    if meta["profile"] == "aligned":
        text_lengths = np.fromfile(path / "text.len", dtype=np.int64)
        durations = np.fromfile(path / "dur.bin", dtype=np.int32)
        if len(text_lengths) != n_samples or len(durations) != int(text_lengths.sum()):
            raise ValueError(f"invalid aligned lengths in {path}")
        if int(meta.get("num_text_tokens", -1)) != int(text_lengths.sum()):
            raise ValueError(f"text count does not match metadata in {path}")
        text = np.fromfile(path / "text.bin", dtype=np.int32)
        if len(text) != len(durations) or np.any(durations <= 0):
            raise ValueError(f"invalid aligned arrays in {path}")
        cursor = 0
        for audio_len, text_len in zip(lengths, text_lengths):
            if int(durations[cursor:cursor + text_len].sum()) != int(audio_len):
                raise ValueError(f"alignment duration mismatch in {path}")
            cursor += int(text_len)
        if [int(row.get("text_tokens", -1)) for row in sample_rows] != text_lengths.tolist():
            raise ValueError(f"sample text lengths do not match in {path}")
    if verify_checksums:
