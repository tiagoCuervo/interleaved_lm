    ...
    *,
    ...
    for path in sorted(root.rglob("*.trans.txt")):
        for line in path.read_text().splitlines():
            ...
            if utterance in transcripts:
                raise ValueError(f"duplicate LibriSpeech transcript ID: {utterance}")
    ...
    for utterance, transcript in sorted(transcripts.items()):
        if utterance not in audio_paths:
            raise FileNotFoundError(f"missing audio file for {utterance}")
        speaker = utterance.split("-", 1)[0]
        yield ManifestRecord(
            id=f"{dataset}:{split}:{utterance}",
            dataset=dataset,
            split=split,
            audio=str(audio_paths[utterance]),
...
def fairseq_tsv_records(
    ...
    *,
    ...
    root = root.resolve()
    transcript_lines = Path(transcripts).read_text().splitlines() if transcripts else []
    if transcript_lines and len(transcript_lines) != len(lines) - 1:
        raise ValueError("transcript file and Fairseq manifest have different lengths")
    for index, line in enumerate(lines[1:]):
        ...
        yield ManifestRecord(
            ...
            transcript=transcript_lines[index] if transcript_lines else None,
            source_revision=source_revision,
        )
    ...
    with Path(path).open() as stream:
        for index, line in enumerate(stream):
            ...
            yield ManifestRecord(
                ...
                transcript=raw.get("transcript", raw.get("text")),
