...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="interleaved-speech-data", description="Prepare versioned speech-unit shards")
    ...
    enc.add_argument("--profile", choices=["units-only", "aligned"], required=True)
    enc.add_argument("--speech-tokenizer", choices=["hubert", "mimi"], required=True)
    enc.add_argument("--hubert-model")
    enc.add_argument("--hubert-revision")
    enc.add_argument("--kmeans", action="append")
    ...
    enc.add_argument("--mimi-repo", default="kyutai/moshiko-pytorch-bf16")
    enc.add_argument("--mimi-revision")
    enc.add_argument("--mimi-filename", default="tokenizer-e351c8d8-checkpoint125.safetensors")
    enc.add_argument("--device", default="cuda")
    enc.add_argument("--text-tokenizer")
    enc.add_argument("--text-tokenizer-revision")
    enc.add_argument("--alignment-source", choices=["timestamps", "mms"], default="timestamps")
    enc.add_argument("--keep-audio", action="store_true")

    tiny = commands.add_parser("tinystories")
    tiny.add_argument("--output", required=True)
    tiny.add_argument("--profile", choices=["units-only", "aligned"], required=True)
    ...
    tiny.add_argument("--speech-tokenizer", choices=["hubert", "mimi"], required=True)
    tiny.add_argument("--hubert-model")
    tiny.add_argument("--hubert-revision")
    tiny.add_argument("--kmeans", action="append")
    ...
    tiny.add_argument("--mimi-repo", default="kyutai/moshiko-pytorch-bf16")
    tiny.add_argument("--mimi-revision")
    tiny.add_argument("--mimi-filename", default="tokenizer-e351c8d8-checkpoint125.safetensors")
    tiny.add_argument("--device", default="cuda")
    tiny.add_argument("--text-tokenizer")
    tiny.add_argument("--text-tokenizer-revision")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "manifest-librispeech":
        ...
        result = encode_shard(
            ...
            speech_tokenizer=args.speech_tokenizer,
            ...
            mimi_revision=args.mimi_revision,
            mimi_filename=args.mimi_filename,
            device=args.device,
            text_tokenizer=args.text_tokenizer,
            text_tokenizer_revision=args.text_tokenizer_revision,
